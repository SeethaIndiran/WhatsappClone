...
import com.example.chattingapp.fragments.StoriesViewFragment
...
class RecentUpdatesAdapter(private val fragment:Fragment):RecyclerView.Adapter<RecentUpdatesAdapter.UpdatesViewHolder>() {

    private var binding: ItemLayoutRecentUpdatesBinding? = null

    inner class UpdatesViewHolder(itemBinding: ItemLayoutRecentUpdatesBinding):
        ...
        holder.itemView.apply {
            ...
            setOnClickListener {
                if(fragment is StatusFragment){
                    fragment.navigateToStoriesViewFragment(user,position)
                }
                if(fragment is StoriesViewFragment){
